...
# normally distributed for simplicity
heart_beats = np.random.normal(loc=70, scale=10, size=10000)
...
for i in range(10):
    stratum_heart_beats = heart_beats[(heart_beats >= strata_bounds[i]) & (
        heart_beats < strata_bounds[i+1])]
    ...
    stratified_sample = np.random.choice(
        stratum_heart_beats, size=len(stratum_heart_beats), p=weights)
...
def f(x): return np.sum(norm(loc=heart_beats, scale=1.0).pdf(x))
...
for i in range(num_iterations):
    ...
    weights = np.array([f(x) for x in samples]) / \
        norm(loc=mu, scale=sigma).pdf(samples)
...
print(
    f"5% Percentile of Heart Beats per Minute using Adaptive Importance Sampling: {percentile_5}")
